...
# file_path = '/Users/irischeng/INF553/Assignment/hw2/task2_dataset.csv'
...
basket_u_b = file_RDD.groupByKey().map(lambda x:(x[0], list(set(x[1])))).filter(lambda s: len(s[1])>k).map(lambda s: s[1])
partition = basket_u_b.getNumPartitions()
# print()
basket_list_u_b = basket_u_b.collect()  #list
# print(basket_u_b.take(2))
