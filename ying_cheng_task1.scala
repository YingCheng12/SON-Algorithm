...
//    val File_path = "/Users/irischeng/INF553/Assignment/hw2/small2.csv"
...
//      println(File_rdd.collect()(0))
      val basket_u_b = File_rdd.reduceByKey(_ ++ _).map(s => s._2.toList).repartition(2) //rdd
      //    println(basket_u_b.getNumPartitions)
      val basket_list_u_b = basket_u_b.repartition(1).collect() //list[list]
      var candidates_u_b = basket_u_b.mapPartitions(x => find_cadidates(x, 2))
    ...
    if (case_number == 2){
      ...
      val header = File.first()
      //    println(header)
      val File_rdd = File.filter(line => line != header).map(s => s.split(",")).map(s => (s(1), Set("'" + s(0) + "'")))
//      println(File_rdd.collect()(0))
      val basket_u_b = File_rdd.reduceByKey(_ ++ _).map(s => s._2.toList).repartition(2) //rdd
      //    println(basket_u_b.getNumPartitions)
      val basket_list_u_b = basket_u_b.repartition(1).collect() //list[list]
      var candidates_u_b = basket_u_b.mapPartitions(x => find_cadidates(x, 4))
      var candidates_u_b_list = candidates_u_b.map(x => x).collect().toSet.toList
